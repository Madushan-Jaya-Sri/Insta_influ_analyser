...
# Permission fix for authentication - focuses only on essential files
set -e
...
class User(UserMixin):
    ...
    def ensure_user_directory():
        ...
        try:
            ...
            if not os.path.exists(USERS_DATA_PATH):
                ...
                try:
                    os.chmod(USERS_DATA_PATH, 0o666)
                except:
                    print("Couldn't set permissions, but continuing")
    ...
    def save_users():
        ...
        try:
            ...
            # Write directly to file to avoid permission issues
            with open(USERS_DATA_PATH, 'w') as f:
...
# Make data directories - NO recursive chmod
...
# Copy to all possible locations - just the file, no chmod
...
echo "Copying fixed user model to container..."
docker cp ~/Insta_influ_analyser/user.py flask_app:/app/app/models/user.py

# Copy users.json to the container
...
# Add an emergency login route without changing existing functionality
echo "Adding emergency login route..."
docker exec -i flask_app bash -c 'cat >> /app/app/routes/auth.py << EOF
...
# Recreate containers as root user
...
echo "If you still have issues, use the emergency login: http://13.126.220.175/auth/emergency-login"
