...
# Create the run_debug.py file
cat > scripts/run_debug.py << 'EOF'
"""
...
# Create a simple Dockerfile for debugging
...
RUN mkdir -p app/uploads app/data app/static app/data/sessions uploads data static
RUN chmod -R 777 app/uploads app/data app/static app/data/sessions uploads data static
...
CMD ["python", "-u", "scripts/run_debug.py"]
EOF

# Create a simple docker-compose.yml file for debugging
...
echo "Created debugging files. You can now run:"
echo "docker-compose -f docker-compose.debug.yml up --build"
echo
echo "This will show detailed logs to help identify the issue." 